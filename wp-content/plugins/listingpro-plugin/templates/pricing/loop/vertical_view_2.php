<?php
	...
	/* vertical view code */
							...
							<div class="col-md-4 col-centered '.get_the_ID().' '.$hotClass.' pricing_plans_v2 '.$classoffset.'">
							...
							echo '<div class="lp-active-badge-on-plan">
									...
									*/
									...
									<div class="'.$classForBg.' version_bottom_padding" style="background-color:#fff;">
										...
										<div class="lp-plane-top-wrape lp_plan_image_heading">
											...
											<a>'.get_the_title().'</a>
											<p class="lp-des-v2">'.get_post_meta(get_the_ID(), 'plan_desc_content', true).'</p>
											...
											echo '<div class="listing_day_duration">';

												echo  '<span>';

												if( !empty($plan_time) ){

													echo  $plan_time.' '.esc_html__('days', 'listingpro-plugin');

												}

												else{

													echo  esc_html__('Unlimited days', 'listingpro-plugin');
												...
												if(!empty($posts_allowed_in_plan) && $plan_type=="Package"){
													...
													echo  '<span>'. $posts_allowed_in_plan.esc_html__(' Listings', 'listingpro-plugin').'</span>';
												...
												if(empty($posts_allowed_in_plan) && $plan_type=="Package"){
													...
													echo  '<span>'.esc_html__('Listings : Unlimited', 'listingpro-plugin').'</span>';
											...
											if(!empty($post_price)){
												...
												echo '<p>'.esc_html__("Free", 'listingpro-plugin').'</p>';
											...
											if(!empty($plan_type_name)){
												...
												echo '<span class="package-type">'.$plan_type_name.'</span>';
											...
											if(is_numeric($plan_limit_left)){
												...
												if( !empty($results) && count($results)>0 ){
													...
													if(!empty($post_price) && $plan_limit_left>0){
														...
														echo  '<span style="font-size:12px;color:#fff" class="allowedListing">'.esc_html__('Remaining Listings : ', 'listingpro-plugin') . $plan_limit_left.'</span>';
									...
									echo '<form method="post" action="'.listingpro_url('submit-listing').'" class="price-plan-button">
											...
											if(empty($post_price) && $plan_type=="Package"){
												...
												if(!empty($plan_limit_left)){
													...
													echo '<input id="submit'.$post->ID.'" class="lp-price-free lp-without-prc btn" type="submit" value="'.esc_html__('Choose Plan', 'listingpro-plugin').'" name="submit">';
											...
											echo  wp_nonce_field( 'price_nonce', 'price_nonce_field'.$post->ID ,true, false );
							...
							<div class="lp-view-all-price-feature"><span class="lp-hide-show-price-features" style="'.$left.'">View All Features</span></div>';





							echo '<div class="section-lp-price-list">';
										...
										echo '<ul class="lp-listprc">
											...
											<li class="outer_tooltip_price">
												...
												if( !empty($plan_time) ){
													...
													echo  esc_html__('Duration', 'listingpro-plugin').' : '.$plan_time.' '.esc_html__('days', 'listingpro-plugin');

												}

												else{

													echo  esc_html__('Duration', 'listingpro-plugin');
												...
												echo  '<div class="tooltip_price_features">
												...
												if(!empty($posts_allowed_in_plan) && $plan_type=="Package"){
													...
													echo '<li class="outer_tooltip_price">';

													echo '<span class="icon-text">'.listingpro_fontawesome_icon('checked').'</span>';

													echo  '<span>'.esc_html__('Max. Listings : ', 'listingpro-plugin'). $posts_allowed_in_plan.'</span>';
												...
												if(empty($posts_allowed_in_plan) && $plan_type=="Package"){
													...
													echo  '<span>'.esc_html__('Max. Listings : Unlimited', 'listingpro-plugin').'</span>';
